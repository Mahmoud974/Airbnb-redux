...
export default function Categories() {
  ...
  const categoriesList = [
    ...
    { name: 'A la pistes', icon: <FaSkiing /> },
    { name: 'Lacs', icon: <GiRiver /> },
    { name: 'Cabanes', icon: <MdCabin /> },
    { name: 'Cabanes', icon: <GiLuckyFisherman /> },
...
if(direction === ScrollDirection.LEFT){
  ...
  return (
    <div className='relative pt-6 px-10 flex items-center gap-3'>
        {scrollPosition > 0 && (
            <button onClick={() => handleScroll(ScrollDirection.LEFT)} className='p-1 bg-white rounded-full border border-black z-10 font-bold flex item-center justify-center text-sm'>
...
<div className="flex items-center">
    {categoriesList.map((category, index) =>{
        return (
            <div key={index} className="flex w-32 flex-shrink-0">
          ...
          {scrollPosition < categoriesList.length * 200 - 400 && (
            <button onClick={() => handleScroll(ScrollDirection.RIGHT)} className='p-1 bg-white rounded-full border border-black z-10 font-bold flex item-center justify-center text-sm'>
